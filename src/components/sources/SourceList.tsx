...
import { FileText, Youtube, Link2, Type, Subtitles, Pencil, Trash2, Loader2, Search } from 'lucide-react';
...
export function SourceList() {
  const { sources, setPreviewSource, setPreviewStartTime, setSidebarOpen, deleteSource, renameSource, toggleSourceSelection, searchQuery } = useApp();
  ...
  if (filtered.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-10 text-center px-2 text-muted-foreground">
        <Search className="h-6 w-6 opacity-30" />
        <div className="text-sm">No sources match "{searchQuery}"</div>
      </div>
    );
  }
